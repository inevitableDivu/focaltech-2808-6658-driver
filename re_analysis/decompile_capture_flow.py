import pefile
import capstone

pe = pefile.PE("ftWbioUmdfDriverV2.dll")
image_base = pe.OPTIONAL_HEADER.ImageBase
text_sec = [s for s in pe.sections if s.Name.startswith(b'.text')][0]
text_data = text_sec.get_data()
text_va = image_base + text_sec.VirtualAddress

cs = capstone.Cs(capstone.CS_ARCH_X86, capstone.CS_MODE_64)

def get_code(va, size):
    offset = va - text_va
    return text_data[offset:offset+size]

# Let's disassemble the image capture functions:
# 0x1800083f8 (called at 0x18000745e)
# and the entire body of 0x1800083f8
def disasm_func_full(start_va, max_insns=120):
    code = get_code(start_va, max_insns * 15)
    print(f"\n=== Function at 0x{start_va:x} ===")
    count = 0
    for insn in cs.disasm(code, start_va):
        print(f"0x{insn.address:x}: {insn.mnemonic:8s} {insn.op_str}")
        count += 1
        if count >= max_insns:
            break
        if insn.mnemonic == 'ret' and count > 15:
            break

disasm_func_full(0x1800083f8, 120)

